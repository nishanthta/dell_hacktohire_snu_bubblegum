...
export default class Dashboard extends Component {
  ...
  moveItem = (item) => {
    ...
    fetch('http://18.188.218.137/move-item?src=delhi&dest=' + item.Suggestion.toLowerCase(), {
      method: 'POST',
      headers: {
      ...
      .then(async (res) => {
        ...
        for (let index in d) {
          ...
          if (data.label == 'PL1') {
            ...
            const suggestions = ['Chennai', 'Delhi', 'Kolkata', 'Mumbai'];
            data.suggestions = suggestions[Math.floor(Math.random() * 5)];
